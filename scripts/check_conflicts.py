...
def load_data():
    ...
    for filepath in glob.glob("dictionary/dictionary.*.db"):
        ...
        for table in CHECK_FIELDS:
            cursor.execute(f"SELECT global_id, {', '.join(CHECK_FIELDS[table])} FROM {table}")
...
def check_categories_consistency(categories_by_lang):
    ...
    mismatches = {lang: len(ids) for lang, ids in categories_by_lang.items() if len(ids) != len(reference_ids)}
    if mismatches:
...
def main():
    ...
    parser.add_argument("--full-report", action="store_true", help="Write detailed reports")
    parser.add_argument("--precommit", action="store_true", help="Run in pre-commit mode")
    ...
    if args.precommit and has_issues:
        answer = input("⚠ Conflicts detected. Do you want to continue with commit? [y/N]: ").lower()
        if answer != "y":
